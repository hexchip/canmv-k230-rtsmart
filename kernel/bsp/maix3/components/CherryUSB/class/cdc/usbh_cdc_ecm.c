...
get_mac:
    ...
    ret = usbh_get_string_desc(cdc_ecm_class->hport, mac_str_idx, (uint8_t *)mac_buffer, sizeof(mac_buffer));
    if (ret < 0) {
        return ret;
    }
...
err_t usbh_cdc_ecm_linkoutput(struct netif *netif, struct pbuf *p)
{
    ...
    if (((p->tot_len) % USB_GET_MAXPACKETSIZE(g_cdc_ecm_class.bulkout->wMaxPacketSize)) == 0) {
        g_cdc_ecm_class.bulkout_urb.transfer_flags = 0x0;
        g_cdc_ecm_class.bulkout_urb.transfer_flags |= URB_ZERO_PACKET;
    }
