 ...
 */
static void dw_i2c_master_init(struct i2c_regs *i2c_base)
{
    /* Disable i2c */
    dw_i2c_enable(i2c_base, 0);
...
#else
            rt_kprintf("error, not enable i2c slave eeprom\n");
#endif
        } else {
            dw_i2c_master_init((struct i2c_regs *)i2c_buses[i].i2c.regs);
